...
`platforms/1.12.2` 是独立的 **GPL-3.0-only、0.2.0-dev** 工程。2026-09-25 已完成正式重混淆 JAR 构建及原生 Java 8 回归；随后[独立专服通过 13 项真实 TCP 检查](legacy-mod-validation-1.12.2-gpl.md)。[真实图形客户端](legacy-client-validation-1.12.2.md)分别连接 Forge 专服和 Paper 1620 插件服通过，严格默认策略下报告获准后持续在线超过 65 秒，四个进程均正常退出。它不属于冻结的 0.1.1 五个发布安装包。
...
当前联机证据覆盖上述固定 Forge / Paper 组合；生产认证、混合端和全部玩法没有通过验收。设备和 VM 信号仍为可伪造的客户端自报。本阶段没有选定或验证 1.12.2 的 Grim、AntiXray 或其他行为／矿物混淆组合。
