...
async function connect(options = {}) {
  ...
  // Observe actual decoded reasons even while an unfinished bundle withholds
  // public packet events. No socket-close or server-log reason inference.
  const detachDisconnects = observeDisconnects(client, observation => {
    state.kicked = observation.reasonJson;
    disconnectObservations.push({name, source:observation.source, packetName:observation.packetName,
      packetState:observation.packetState, clientState:observation.clientState, reasonJson:observation.reasonJson});
  });
  ...
  client.on('end',r=>{state.ended=true;state.endReason=String(r);active.delete(client);detachDisconnects();});
  ...
  attachPositionAcknowledgements(client, mcData.supportFeature('sendsPlayerLoadedPacket'));
  ...
  if (name === 'QVBot14' || name === 'CheatAccount') {
    metadataTraces.push(createMetadataTrace(client, name, () => ({
      joined:state.joined, sent:state.sent, ended:state.ended,
      kickedPresent:state.kicked.length > 0, errorPresent:state.error.length > 0
    })));
  }
...
async function main() {
  ...
  const result={passed:cases.length,cases,orePalette:[...orePalette],maxChunkZeroPadding,disconnectObservations,kind:'real TCP clients using synthetic self-reports; not rendered mod client or real VM',version};
...
main().catch(error=>{console.error(error);process.exitCode=1;}).finally(async()=>{
  for(const client of active)client.end('test finished');
  await pause(300);
  // Retain decoder provenance on failed QA attempts as well as successful ones.
  fs.writeFileSync(path.join(folder,'protocol-disconnect-observations.json'),JSON.stringify({version,disconnectObservations},null,2));
  fs.writeFileSync(path.join(folder,'protocol-metadata-trace.json'),JSON.stringify({version,scope:'Diagnostic metadata only; no reason inference or assertion changes',traces:metadataTraces.map(trace=>trace.finish())},null,2));
});
