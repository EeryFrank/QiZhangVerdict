...
public final class CommandParserSmoke {
    public static void main(String[] args) {
        // The permission codec in 1.21.11 references the registered game types.
        ...
        if (!dispatcher.parse("qzverdict status", source(2)).getReader().canRead()) throw new AssertionError("Gamemaster unexpectedly parsed administrator command");
        if (dispatcher.parse("qzverdict status", source(3)).getReader().canRead()) throw new AssertionError("Administrator permission was rejected");
    ...
    private static CommandSourceStack source(int permission) {
        return new CommandSourceStack(CommandSource.NULL, Vec3.ZERO, Vec2.ZERO, null, net.minecraft.server.permissions.LevelBasedPermissionSet.forLevel(net.minecraft.server.permissions.PermissionLevel.byId(permission)), "ParserSmoke", Component.literal("ParserSmoke"), null, null);
    }
