...
def run_phase(args, folder, phase):
    ...
    try:
        with log.open('w', encoding='utf-8') as output:
            process = subprocess.Popen([args.java, '-Xms256M', '-Xmx1536M', '-jar', 'server.jar', getattr(args, 'server_nogui', '--nogui')], cwd=folder,
            ...
            if phase == 'trigger' and getattr(args, 'setup_commands', None):
                for command in args.setup_commands:
                    process.stdin.write(command + '\n'); process.stdin.flush()
                    # Optional legacy fixture commands may need a tick to load a forced chunk.
                    time.sleep(getattr(args, 'setup_command_pause', 0))
                process.stdin.write('say QV_SCENE_READY\n'); process.stdin.flush()
                deadline = time.monotonic() + 30
                while 'QV_SCENE_READY' not in log.read_text('utf-8', errors='replace'):
                    if process.poll() is not None: raise RuntimeError('Server exited while preparing the controlled scene')
                    if time.monotonic() > deadline: raise TimeoutError('Controlled scene commands did not complete')
                    time.sleep(.1)
            protocol_path = Path(getattr(args, 'protocol', ROOT/'scripts/grim_link_protocol.cjs'))
            result['executedProtocolSha256'] = digest(protocol_path)
            with (folder / ('protocol-' + phase + '.log')).open('w', encoding='utf-8') as test_output:
                tests = subprocess.Popen([args.node, str(protocol_path), args.version,
            ...
            expected_cases = getattr(args, 'expected_cases', {}).get(phase, 6)
            result['passed'] = result['protocol']['passed'] and result['protocol'].get('caseCount') == expected_cases
