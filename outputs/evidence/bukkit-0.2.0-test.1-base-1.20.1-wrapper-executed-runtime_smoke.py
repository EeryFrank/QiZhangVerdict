...
def run(version, mode, grim_path=None, compression=256, guard_path=None, expected_guard_sha256=None):
    ...
    folder.mkdir(parents=True, exist_ok=(mode == "startup"))
    ...
    with log.open("w", encoding="utf-8") as output:
        ...
        process = subprocess.Popen([str(java), "-Xms256M", "-Xmx1536M", "-jar", "server.jar", "--nogui"], cwd=folder, stdin=subprocess.PIPE, stdout=output, stderr=subprocess.STDOUT, text=True, creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
        try:
            ...
            if mode != "startup":
                ...
                with (folder / "protocol-results.log").open("w", encoding="utf-8") as test_log:
                    tests = subprocess.Popen([str(NODE), str(PROJECT / "scripts/protocol_smoke.cjs"), version, str(port), str(folder)], env=env, stdout=test_log, stderr=subprocess.STDOUT, creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
