...
def candidate_arguments(parser, args):
    ...
    if supplied and args.action not in ('run', 'stage'):
        parser.error('Candidate JAR overrides are allowed only for run/stage; prepare/install retain historical inputs')
    overrides = {}
    ...
    if args.action == 'run' or (args.action == 'stage' and supplied):
        try:
            new_run_directory(CACHE / 'candidate-staging' if args.action == 'stage' else CACHE, args.run_name)
        except (ValueError, OSError) as error:
            parser.error(str(error))
    return overrides


def memory_gate(action, minimum_gib=5):
    ...
    available = status.available_phys / (1024 ** 3)
    record = {'action': action, 'minimum_available_gib': minimum_gib, 'available_gib': round(available, 3),
              'utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'allowed': available >= minimum_gib}
    CACHE.mkdir(parents=True, exist_ok=True)
    with (CACHE / 'resource-gate.jsonl').open('a', encoding='utf-8') as output:
        output.write(json.dumps(record) + '\n')
    if not record['allowed']:
        raise RuntimeError('No Java started: available physical memory below ' + str(minimum_gib) + ' GiB: ' + str(record['available_gib']))
    return record
...
def install():
    memory_gate('official-installer')
    directory = CACHE / 'client'
...
def stage(candidate_run_name=None):
    ...
    if candidate_run_name is not None:
        # Candidate staging is a new receipt/game directory, never a rewrite of the base plan.
        directory = new_run_directory(CACHE / 'candidate-staging', candidate_run_name)
        directory.mkdir(parents=True, exist_ok=False)
        game = directory / 'client-game'
        (game / 'mods').mkdir(parents=True)
        plan = {**plan, 'game': str(game)}
    shutil.copy2(GUARD, Path(plan['game']) / 'mods' / GUARD.name)
    if matrix.digest(Path(plan['game']) / 'mods' / GUARD.name) != GUARD_SHA:
    ...
    matrix.save(directory / 'launch-plan.json', plan)
    failed = CACHE / 'client/client-install-result.json'
...
def prepare_server(run_name, kind, include_guard=True):
...
def run(run_name, kind, video_preset='vanilla', forge_splash='default', candidate_override=False):
    ...
    # A new writable client game directory prevents old options/configuration or
    # screenshots from leaking into a comparison; libraries/assets remain pinned.
    game = directory / 'client-game'
    (game / 'mods').mkdir(parents=True)
    shutil.copy2(GUARD, game / 'mods' / GUARD.name)
    ...
    if candidate_override:
        plan.update({'guard_source': str(GUARD), 'guard_sha256': GUARD_SHA})
    ...
    try:
        ...
        result['automated_runtime_passed'] = result['passed']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--guard-jar', help='Candidate client/mod JAR; requires --guard-sha256 (run/stage only)')
    parser.add_argument('--guard-sha256', help='Expected candidate mod SHA-256, exactly 64 hexadecimal characters')
    parser.add_argument('--bukkit-jar', help='Candidate Bukkit JAR; requires --bukkit-sha256 (run/stage only)')
    ...
    elif args.action == 'stage': stage(args.run_name if overrides else None)
    else: run(args.run_name, args.server_kind, args.video_preset, args.forge_splash, candidate_override=bool(overrides))
