...
def pin_guard(profile):
    loader, mc = profile.split('-', 1)
    receipt = CACHE / mc / 'builds' / ('02' if mc == '1.19.2' else '01') / 'result.json'
    built = read(receipt)
    if not built['buildSuccessful']: raise ValueError('No successful pinned build receipt')
...
def stage_base(profile, name):
    ...
    try:
        ...
        result.update(minecraft=mc, loader=loader, loaderVersion=installed['loaderVersion'], design=design,
                      installer=installed['installer'], guard=pin_guard(profile), java=str(JAVA17), javaMajor=17,
...
def main():
    ...
    if args.action == 'stage-base': stage_base(args.profile, args.base_name)
