...
review=BASE/'client-preparation-review-01';review.mkdir(exist_ok=False)
checks=[]
...
with unittest.mock.patch.object(qa.subprocess,'Popen',fail_if_process),unittest.mock.patch.object(qa.subprocess,'run',fail_if_process):
 ...
 check('other UUID device association is rejected',not runtime.associated(server))
 state.write_text('D\t'+runtime.PLAYER_UUID+'\t'+'a'*64+'\n')
 check('exact synthetic UUID association is accepted',runtime.associated(server))
...
summary={'schemaVersion':1,'prepared':True,'staticReviewPassed':True,'staticChecks':checks,'staticCheckCount':len(checks),
         ...
         'boundary':'Official inputs/assets and launch-plan inspection only; Forge/Neo client installers and all game/client/visual acceptance remain unexecuted.',
