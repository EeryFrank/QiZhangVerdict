...
def run(target, attempt):
    ...
    cmd = [str(project/'gradlew.bat'), '--no-daemon', '--max-workers=1', '--console=plain', '--stacktrace', '--project-cache-dir', str(folder/'project-cache'), '-Dorg.gradle.jvmargs=-Xmx1536M -Dfile.encoding=UTF-8', '-Porg.gradle.java.installations.paths='+','.join(x.as_posix() for x in (JDK17, JDK21)), '-Porg.gradle.java.installations.auto-detect=false', '-Porg.gradle.java.installations.auto-download=false', '-PqzRuntimeRoot='+(folder/'runtime').as_posix(), '-PqzTestRoot='+(folder/'tests').as_posix(), ':neoforge:payloadCodecSmoke', ':neoforge:clientChallengeDispatchSmoke']
    old = {p.relative_to(ROOT).as_posix():sha(p) for p in ROOT.rglob('build/libs/qizhangverdict-*.jar') if True}
