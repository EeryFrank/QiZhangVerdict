...
def stage(args):
    ...
    with zipfile.ZipFile(anti) as jar:
        ...
        assert get_json_from_jar(jar, "fabric.mod.json")["version"] == "1.1.0"
        assert jar.testzip() is None
    ...
    generator = {"layers": [{"block": "minecraft:bedrock", "height": 1}, {"block": "minecraft:stone", "height": 59},
                             {"block": "minecraft:grass_block", "height": 1}], "biome": "minecraft:plains", "structures": {"structures": {}}}
    properties = {
        "server-ip": "127.0.0.1", "server-port": args.port, "online-mode": "false", "enable-rcon": "false",
        "level-name": "world", "level-type": "minecraft:flat", "generator-settings": json.dumps(generator, separators=(",", ":")),
...
def run(args):
    ...
    result = {"schemaVersion": 1, "passed": False, "profile": marker["profile"], "harness": record(__file__),
              "guardSha256": GUARD_SHA, "antiXraySha256": ANTI_SHA, "java": str(JAVA), "phaseResults": []}
    ...
    try:
        ...
        result["loadedComponents"] = {"antixray": "antixray 1.1.0" in startup, "verdict": "qizhangverdict 0.2.0-dev" in startup,
