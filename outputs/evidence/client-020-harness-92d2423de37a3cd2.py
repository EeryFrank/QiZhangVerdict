...
PROFILES = {
    ...
    "fabric-1.16.5": {"loader": "fabric", "mc": "1.16.5", "version": "0.16.14", "port": 25641,
                      "java_major": 8, "fresh_world": True, "minimum_online_seconds": 65, "offline_social_fallback": True,
...
def client_command(name, plan):
    ...
    if plan.get("offline_services_url"):
        # Authlib 2.1.28 exposes these per-process environment properties. A
        # reserved, non-listening loopback socket causes its stock offline
        # fallback for this synthetic fixture. No game/library bytes are patched.
        command[1:1] = ["-Dminecraft.api.env=QIZHANG_LOCAL_QA",
                       ...
                       "-Dminecraft.api.services.host=" + plan["offline_services_url"]]
...
def run(name, run_name):
    ...
    try:
        with server_log.open("xb") as server_out, client_log.open("xb") as client_out:
            ...
            if cfg.get("offline_social_fallback"):
                offline_socket = socket.socket()
                if hasattr(socket, "SO_EXCLUSIVEADDRUSE"):
                    offline_socket.setsockopt(socket.SOL_SOCKET, socket.SO_EXCLUSIVEADDRUSE, 1)
                offline_socket.bind(("127.0.0.1", 0))
                # Intentionally never listen: reserve the port against reuse.
                address = offline_socket.getsockname()
                with socket.socket() as check:
                    check.settimeout(1)
                    if check.connect_ex(address) == 0:
                        raise RuntimeError("Offline authlib fixture must not have a listener")
                plan["offline_services_url"] = "http://127.0.0.1:" + str(address[1])
                result["authentication_fixture"] = {
                    "kind": "Stock authlib 2.1.28 OfflineSocialInteractions fallback",
                    "services_url": plan["offline_services_url"], "reserved_non_listening_loopback_port": True,
                    "synthetic_access_token": "0", "official_auth_account_session_hosts_unchanged": True,
                    "game_or_library_patched": False, "proves_online_account_authentication": False,
                    "process_properties": [value for value in client_command(name, plan) if value.startswith("-Dminecraft.api.")]}
