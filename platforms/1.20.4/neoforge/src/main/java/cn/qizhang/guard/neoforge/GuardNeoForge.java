...
public final class GuardNeoForge {
    ...
    private void network(RegisterPayloadHandlerEvent event) {
        ...
        event.registrar("qzguard").versioned("1").optional().play(GuardPayload.ID, GuardPayload::read,
            (payload, context) -> context.workHandler().execute(() -> context.player().ifPresent(player -> {
                if (player instanceof ServerPlayer serverPlayer) {
                    if (guard != null) guard.report(serverPlayer, payload.data());
                } else if (FMLEnvironment.dist.isClient()) GuardNeoForgeClient.receive(payload.data(), player);
            })));
    }
