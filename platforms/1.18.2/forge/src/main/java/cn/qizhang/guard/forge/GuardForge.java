...
public final class GuardForge {
    ...
    private net.minecraft.server.MinecraftServer activeServer;
    public GuardForge() {
    ...
    private void serverboundPayload(NetworkEvent.ClientCustomPayloadEvent event) {
        ...
        if (buffer.readableBytes() > 30000) { context.enqueueWork(() -> { var p = context.getSender(); if (p != null) p.connection.disconnect(new net.minecraft.network.chat.TextComponent("QiZhangVerdict payload too large")); }); return; }
    ...
    private void joined(PlayerEvent.PlayerLoggedInEvent event) { if (event.getPlayer() instanceof ServerPlayer p && guard != null) guard.joined(p); }
    private void left(PlayerEvent.PlayerLoggedOutEvent event) { if (event.getPlayer() instanceof ServerPlayer p && guard != null) guard.disconnected(p); }
    private void tick(TickEvent.ServerTickEvent event) { if (event.phase == TickEvent.Phase.END && guard != null && activeServer != null) guard.tick(activeServer); }
