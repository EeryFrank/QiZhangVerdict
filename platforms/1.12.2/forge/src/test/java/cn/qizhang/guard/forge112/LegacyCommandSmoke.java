...
public final class LegacyCommandSmoke {
    public static void main(String[] args) {
        ...
        require(!command.checkPermission(null, sender(2)), "non-administrator may manage guard");
        require(command.checkPermission(null, sender(3)), "administrator rejected");
    ...
    private static net.minecraft.command.ICommandSender sender(final int level) {
        return (net.minecraft.command.ICommandSender) java.lang.reflect.Proxy.newProxyInstance(
                ...
                (proxy, method, values) -> {
                    if ("canUseCommand".equals(method.getName())) return ((Integer) values[0]) <= level;
