...
function inspectChunk(p, player, sky) {
  ...
  try {
    const rawFile = path.join(folder, 'ore-chunk-' + player + '.json');
    if (!fs.existsSync(rawFile)) fs.writeFileSync(rawFile, JSON.stringify({version, x:p.x,z:p.z,bitMap:p.bitMap,
      groundUp:p.groundUp,skyLightSent:sky,bytes:p.chunkData.length,sha256:hash(p.chunkData),
      base64:p.chunkData.toString('base64'), scope:'Controlled synthetic QA world network payload; not a game distribution binary.'},null,2));
    const chunk = new Chunk(); chunk.load(p.chunkData, p.bitMap, sky, p.groundUp);
    const hidden = chunk.getBlock(new Vec3(8,32,8));
    const exposed = chunk.getBlock(new Vec3(12,32,8));
    const control = chunk.getBlock(new Vec3(9,32,8));
