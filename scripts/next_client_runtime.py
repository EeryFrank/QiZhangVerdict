...
def validate_player_name(name):
    if not isinstance(name, str) or not re.fullmatch(r'[A-Za-z0-9_]{1,16}', name):
        raise ValueError('Synthetic Minecraft username must contain 1 to 16 ASCII letters, digits or underscores')
    return name
...
def uuid_has_device_shape(server):
...
def windows_installation_id():
    """Return the private x64 registry value only to an in-memory comparator."""
    try:
        import winreg
        ...
        return value if kind == winreg.REG_SZ and re.fullmatch(r'[a-f0-9-]{16,64}', value) else ''
    except (OSError, ImportError, ValueError):
        return ''


def device_association(server, game, client_log):
    """Recompute ClientReporter.deviceId privately; expose no ID, scope or digest."""
    result = {'uuidShapeAssociation': False, 'matchingUuidDeviceRowCount': 0, 'expectedDigestMatch': False,
              ...
              'algorithm': 'ClientReporter SHA-256 of QiZhangVerdict|lowercase trimmed server scope|lowercase trimmed installation ID',
              'privateIdentifiersExposed': False}
    state = server / 'config/qizhangverdict/accounts.state'
    scope_path = server / 'config/qizhangverdict/server-id.txt'
...
def run(profile, base_name, run_name, port):
    ...
    try:
        ...
        with server_log.open('xb') as server_out, client_log.open('xb') as client_out:
                ...
                while time.monotonic() < deadline and server.poll() is None and client.poll() is None:
                    ...
                    if mode and online and status:
                        return {'queryByteOffset': offset, 'survivalLine': mode, 'onlineLine': online, 'strictStatusLine': status,
                                'deviceAssociation': device_association(server_dir, game, client_log)}
                    time.sleep(.2)
                raise RuntimeError('Fresh post-query suffix did not prove Survival, online player and strict status')
            deadline = time.monotonic() + 240
            while time.monotonic() < deadline and server.poll() is None and client.poll() is None:
                windows(client.pid, 'hide')
                if PLAYER + ' joined the game' in text(server_log) and uuid_has_device_shape(server_dir): break
            ...
            first = fresh_query()
            if not first['deviceAssociation']['expectedDigestMatch']: raise RuntimeError('Persisted device does not match the actual client installation ID and current server scope')
            ...
            if not last['deviceAssociation']['expectedDigestMatch']: raise RuntimeError('Final device association failed independent scoped digest recomputation')
